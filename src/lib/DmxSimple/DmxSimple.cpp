 ...
 * Modified again by Otto Urpelainen (2016) to add support for Attiny45
 * microprocessor.
 */
#include "../../config.h"
 ...
 */
    ...
    #define TIMER_INTERRUPT_ENABLE() TIMSK2 |= _BV(TOIE2)
    #define TIMER_INTERRUPT_DISABLE() TIMSK2 &= ~_BV(TOIE2)
    #define INTERRUPT_VECTOR TIMER2_OVF_vect
#elif defined(__AVR_ATmega8__)
    #define TIMER_INTERRUPT_ENABLE() TIMSK |= _BV(TOIE2)
    #define TIMER_INTERRUPT_DISABLE() TIMSK &= ~_BV(TOIE2)
    #define INTERRUPT_VECTOR TIMER2_OVF_vect
#elif defined(__AVR_ATtiny45__)
    #define TIMER_INTERRUPT_ENABLE() TIMSK |= _BV(TOIE0)
    #define TIMER_INTERRUPT_DISABLE() TIMSK &= ~_BV(TOIE0)
    #define INTERRUPT_VECTOR TIMER0_OVF_vect
#else
...
void dmxBegin()
{
  ...
  // Note: times configured differently, fires every 256*256 clock cycles now
 ...
 * own routine, so the TIMER interrupt is disabled for the duration of
 * the service routine.
 */
ISR(INTERRUPT_VECTOR,ISR_NOBLOCK) {
...
void dmxWrite(int channel, uint8_t value) {
  ...
  if ((channel > 0) && (channel <= DMX_SIZE)) {
    ...
    dmxMax = channel > dmxMax ? channel : dmxMax;
...
void dmxMaxChannel(int channel) {
  if (channel <=0) {
    ...
    dmxMax = channel > DMX_SIZE ? DMX_SIZE : channel;
    if (!dmxStarted) dmxBegin();
  }
}
